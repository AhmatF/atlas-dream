import Link from 'next/link';

interface FooterProps {
  lang: string;
}

const footerContent = {
  en: {
    tagline: 'Luxury & authenticity in Marrakech',
    services: {
      title: 'Services',
      links: [
        { name: 'Car Rental', href: '/en/cars' },
        { name: 'Villas', href: '/en/villas' },
        { name: 'Concierge', href: '/en/concierge' },
      ],
    },
    company: {
      title: 'Company',
      links: [
        { name: 'Blog', href: '/en/blog' },
        { name: 'About', href: '/en/about' },
        { name: 'Contact', href: '/en/contact' },
      ],
    },
    contact: {
      title: 'Contact',
      phone: '+212 774 885 461',
      email: 'contact@atlasdream.ma',
      whatsapp: 'WhatsApp',
    },
    legal: {
      copyright: '© 2025 Atlas Dream. All rights reserved.',
      privacy: 'Privacy Policy',
      terms: 'Terms of Service',
    },
  },
  fr: {
    tagline: 'Le luxe & l\'authentique à Marrakech',
    services: {
      title: 'Services',
      links: [
        { name: 'Location de voiture', href: '/fr/cars' },
        { name: 'Villas', href: '/fr/villas' },
        { name: 'Conciergerie', href: '/fr/concierge' },
      ],
    },
    company: {
      title: 'Entreprise',
      links: [
        { name: 'Blog', href: '/fr/blog' },
        { name: 'À propos', href: '/fr/about' },
        { name: 'Contact', href: '/fr/contact' },
      ],
    },
    contact: {
      title: 'Contact',
      phone: '+212 774 885 461',
      email: 'contact@atlasdream.ma',
      whatsapp: 'WhatsApp',
    },
    legal: {
      copyright: '© 2025 Atlas Dream. Tous droits réservés.',
      privacy: 'Politique de confidentialité',
      terms: 'Conditions d\'utilisation',
    },
  },
};

export default function Footer({ lang }: FooterProps) {
  const content = footerContent[lang as 'en' | 'fr'] || footerContent.en;
  const whatsappNumber = '+212774885461';
  const whatsappMessage = lang === 'en'
    ? 'Hello, I would like to get more information about Atlas Dream services.'
    : 'Bonjour, je souhaite obtenir plus d\'informations sur les services Atlas Dream.';
  const whatsappLink = `https://wa.me/${whatsappNumber.replace('+', '').replace(/\s/g, '')}?text=${encodeURIComponent(whatsappMessage)}`;

  return (
    <footer className="bg-[var(--color-ebony)] text-white">
      <div className="container mx-auto px-6 py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
          {/* Brand */}
          <div className="lg:col-span-1">
            <h3 className="text-2xl font-bold mb-4" style={{ fontFamily: 'var(--font-cormorant)' }}>
              Atlas Dream
            </h3>
            <p className="text-gray-400 text-sm leading-relaxed">
              {content.tagline}
            </p>
          </div>

          {/* Services */}
          <div>
            <h4 className="font-semibold mb-4">{content.services.title}</h4>
            <ul className="space-y-2">
              {content.services.links.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-gray-400 hover:text-[var(--color-brass)] transition-colors text-sm"
                  >
                    {link.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Company */}
          <div>
            <h4 className="font-semibold mb-4">{content.company.title}</h4>
            <ul className="space-y-2">
              {content.company.links.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="text-gray-400 hover:text-[var(--color-brass)] transition-colors text-sm"
                  >
                    {link.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h4 className="font-semibold mb-4">{content.contact.title}</h4>
            <ul className="space-y-3">
              <li>
                <a
                  href={`tel:${content.contact.phone.replace(/\s/g, '')}`}
                  className="text-gray-400 hover:text-[var(--color-brass)] transition-colors text-sm flex items-center gap-2"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                  {content.contact.phone}
                </a>
              </li>
              <li>
                <a
                  href={`mailto:${content.contact.email}`}
                  className="text-gray-400 hover:text-[var(--color-brass)] transition-colors text-sm flex items-center gap-2"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                  {content.contact.email}
                </a>
              </li>
              <li>
                <a
                  href={whatsappLink}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-gray-400 hover:text-[var(--color-brass)] transition-colors text-sm flex items-center gap-2"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                  </svg>
                  {content.contact.whatsapp}
                </a>
              </li>
            </ul>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="border-t border-gray-800 mt-12 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-gray-400 text-sm">{content.legal.copyright}</p>
          <div className="flex gap-6">
            <Link href={`/${lang}/privacy`} className="text-gray-400 hover:text-[var(--color-brass)] transition-colors text-sm">
              {content.legal.privacy}
            </Link>
            <Link href={`/${lang}/terms`} className="text-gray-400 hover:text-[var(--color-brass)] transition-colors text-sm">
              {content.legal.terms}
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
